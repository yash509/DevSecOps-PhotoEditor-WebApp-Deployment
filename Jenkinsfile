pipeline {
    ...
    stages {
        ...
        stage('Checkout from Git') {                        
            steps {                                       
                git branch: 'main', url: 'https://www.github.com/yash509/DevSecOps-PhotoEditor-WebApp-Deployment.git'
            }
        }
        ...
        stage("Docker Image Tagging"){
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                            sh "docker tag pe-wa yash5090/pe-wa:latest " 
                        //}
                    }
                }
            }
        ...
        stage('Docker Image Scanning') { 
            steps { 
                sh "trivy image --format table -o trivy-image-report.html yash5090/pe-wa:latest" 
            } 
        } 
        ...
        stage("Image Push to DockerHub") {
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker') {
                            sh "docker push yash5090/pe-wa:latest "
                        //}
                    }
                }
            }
        ...
        stage('Docker Scout Image Scanning') {
            steps {
                script{
                   withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                       sh 'docker-scout quickview yash5090/pe-wa:latest'
                       sh 'docker-scout cves yash5090/pe-wa:latest'
                       sh 'docker-scout recommendations yash5090/pe-wa:latest'
                       sh 'docker-scout attestation yash5090/pe-wa:latest'
                   }
                }   
            }
        }
        ...
        stage("TRIVY"){
            steps{
                //dir('Band Website') {
                    sh "trivy image yash5090/pe-wa:latest > trivyimage.txt"   
                //}
            }
        }
        ...
        stage('Docker Scout Artifacts Analysis') {
            steps {
                script{
                  withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                      sh 'docker-scout sbom --format list yash5090/pe-wa:latest' // docker-scout sbom yash5090/pe-wa:latest
                    }
                }   
            }
        }
        ...
        stage ("Remove Docker Container") {
            steps{
                sh "docker stop pe-wa | true"
                sh "docker rm pe-wa | true"
             }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh 'docker run -d --name pe-wa -p 5000:80 yash5090/pe-wa:latest' 
                //}
            }
        }
